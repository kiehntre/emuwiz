...
| 6 | **Mods containing only original patch data** (IPS/BPS/xdelta deltas, PNACH text, TOML/YAML patches) | **Lowest content risk** | Patch formats such as BPS/IPS normally distribute differences rather than complete game images, substantially reducing redistribution risk, but individual mod licensing and content still need to be respected. This is why the ROM-hacking community distributes patches rather than pre-patched images `[COMMUNITY]`. Caveat: the *patch* is still the author's copyrighted work — the conduit posture (#1–3) still applies to it. This is the recommended Phase 1 class. |
