...
| Feature | Backend/component | GUI v2 home | Primary action | User-visible result | Background activity | Failure/recovery | Migration |
|---|---|---|---|---|---|---|---|
| Per-game cheats/mod discovery and preview | `patch_manager`, GUI cheat journey | Mods & Cheats → Game | Choose a game / open its improvements | Applicable providers, candidates, destinations | Existing catalogue/profile lookup | Explain unavailable/ambiguous evidence; review setup | Native chooser and shared workflow |
| PCSX2 cheats and texture packs | `pcsx2_*`, `pcsx2_texture_pack` | Mods & Cheats → PlayStation 2 → Cheats / Texture Packs | Preview pack | Existing verified destination and change plan | Existing inspection/staging | Correct serial/profile; cancel/rollback | Cheats native; existing texture-pack panel retained |
...
| Dolphin Gecko/AR/OnFrame cheats and texture packs | `dolphin_*`, `gecko_*`, `bsfree_*` | Mods & Cheats → GameCube/Wii → Cheats / Textures | Preview improvement | Existing game-ID-bound plan | Existing retrieval and inspection | Disambiguate codes/profile; transactional undo | Cheats native; existing texture-pack panel retained |
| Xenia patches | `xenia_*` | Mods & Cheats → Xbox 360 → Patches | Preview patch | Title-ID-bound proposal | Existing provider/local lookup | Explain compatibility/identity conflict; undo | Native through the shared cheat workflow |
| RetroArch cheats | `retroarch_cheat_*`, `retroarch_materialization` | Mods & Cheats → Game → RetroArch Cheats | Preview codes | Existing core/profile-compatible plan | Existing discovery/catalogue read | Choose correct profile; preserve existing files | Native through the shared cheat workflow |
...
| Local cheat import and format compatibility | `user_cheat_import`, `cheat_conversion`, `cheat_ir` | Mods & Cheats → Game → Import | Preview local file | Parsed compatible codes and safety findings | Existing bounded parse | Correct type; no invented conversion support | Native through the existing import panel |
| CheatBase, GameHacking, BSFree, Gecko and other registered cheat sources | `cheat_source_registry`, providers, caches | Mods & Cheats → Find improvements; Sources → Cheats | Search / explicitly refresh | Candidates and source status | Existing fetch/cache progress | Retry/offline cache/manual supported import | Native per-game workflow; source administration remains under Sources |
| Cheat reconciliation, activation, uninstall and rollback | `cheat_reconciliation_plan`, `shared_transaction`, `cheat_history` | Game → Installed improvements; History → Cheat activity | Preview change | Exact before/after state | Existing journalled operation | Explicit approval; conflict-safe rollback | Native result/undo and History projection |
