...
fn read_title_id(path: &Path) -> Result<String, Rpcs3OrdinaryModError> {
    let metadata = fs::symlink_metadata(path).map_err(|e| {
        error(
            ...
            format!("PARAM.SFO cannot be read: {e}"),
        )
    })?;
    if metadata.file_type().is_symlink() || !is_safe_regular_file(&metadata) {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::UnsafeMember,
            "PARAM.SFO is not a regular file",
        ));
    }
    if metadata.len() > RPCS3_ORDINARY_MOD_MAX_SFO_BYTES {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::ResourceLimit,
            "PARAM.SFO exceeds the inspection limit",
        ));
    }
    let bytes = fs::read(path)
        .map_err(|e| error(Rpcs3OrdinaryModErrorKind::MalformedIdentity, e.to_string()))?;
    let sfo = parse_param_sfo(&bytes).ok_or_else(|| {
        error(
    ...
    let value = match sfo.get("TITLE_ID") {
        ...
        _ => {
            return Err(error(
                Rpcs3OrdinaryModErrorKind::MissingIdentity,
                "PARAM.SFO has no textual TITLE_ID",
            ));
        }
    };
    if value.len() != 9 || !value.bytes().all(|byte| byte.is_ascii_alphanumeric()) {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::MalformedIdentity,
            "PARAM.SFO TITLE_ID is not a valid PS3 title ID",
        ));
    }
    Ok(value)
}
...
fn locate_root(source_path: &Path) -> Result<PathBuf, Rpcs3OrdinaryModError> {
    if !source_path.is_absolute() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::SourceNotFound,
            "RPCS3 mod source path must be absolute",
        ));
    }
    let metadata = fs::symlink_metadata(source_path)
        .map_err(|e| error(Rpcs3OrdinaryModErrorKind::SourceNotFound, e.to_string()))?;
    if metadata.file_type().is_symlink() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::SourceSymlink,
            "RPCS3 mod source is a symlink",
        ));
    }
    if !metadata.is_dir() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::SourceNotRegular,
            "RPCS3 mod source must be a folder or ZIP",
        ));
    }
    ...
    {
        let entry =
            entry.map_err(|e| error(Rpcs3OrdinaryModErrorKind::SourceNotFound, e.to_string()))?;
        let metadata = fs::symlink_metadata(entry.path())
            .map_err(|e| error(Rpcs3OrdinaryModErrorKind::SourceNotFound, e.to_string()))?;
        if metadata.file_type().is_symlink() {
            return Err(error(
                Rpcs3OrdinaryModErrorKind::UnsafeMember,
                "wrapper contains a symlink",
            ));
        }
    ...
    match roots.len() {
        ...
        0 => Err(error(
            Rpcs3OrdinaryModErrorKind::MissingIdentity,
            "no unambiguous PARAM.SFO identity was found",
        )),
        _ => Err(error(
            Rpcs3OrdinaryModErrorKind::AmbiguousWrapper,
            "package contains multiple possible RPCS3 mod roots",
        )),
    }
}
...
fn inspect_root(
    source_path: &Path,
    root: &Path,
) -> Result<Rpcs3OrdinaryModInspection, Rpcs3OrdinaryModError> {
    let candidates = identity_candidates(root);
    if candidates.is_empty() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::MissingIdentity,
            "RPCS3 mod must contain PARAM.SFO or PS3_GAME/PARAM.SFO",
        ));
    }
    ...
    if ids.len() != 1 {
        return Err(error(
            ...
            "package contains conflicting PS3 TITLE_ID evidence",
        ));
    }
    let declared_title_id = ids.into_iter().next().expect("one identity checked");
    let identity_relative = candidates[0]
        .strip_prefix(root)
        .map_err(|_| {
            error(
                Rpcs3OrdinaryModErrorKind::UnsafeMember,
                "identity escaped package root",
            )
        })?
        .to_path_buf();
    ...
    fn walk(
        ...
        for entry in entries {
                ...
                || (!is_safe_regular_file(&metadata) && !metadata.is_dir())
            {
                return Err(error(
                    Rpcs3OrdinaryModErrorKind::UnsafeMember,
                    format!("unsafe RPCS3 mod member: {}", path.display()),
                ));
            }
            if metadata.is_dir() {
                    ...
                    .any(|component| !matches!(component, Component::Normal(_)))
                {
                    return Err(error(
                        Rpcs3OrdinaryModErrorKind::UnsafeMember,
                        "package member contains unsafe path components",
                    ));
                }
                if files.len() >= RPCS3_ORDINARY_MOD_MAX_FILES {
                    return Err(error(
                        Rpcs3OrdinaryModErrorKind::ResourceLimit,
                        "RPCS3 mod file-count limit reached",
                    ));
                }
                *total = total.saturating_add(metadata.len());
                if *total > RPCS3_ORDINARY_MOD_MAX_TOTAL_BYTES {
                    return Err(error(
                        Rpcs3OrdinaryModErrorKind::ResourceLimit,
                        "RPCS3 mod expanded-size limit reached",
                    ));
                }
                files.push(relative);
            }
    ...
    files.retain(|path| {
        path != &identity_relative
            && path != Path::new("PARAM.SFO")
            && path != Path::new("PS3_GAME/PARAM.SFO")
    });
    if files.is_empty() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::EmptyPackage,
            "RPCS3 mod contains no installable files",
        ));
    }
    files.sort();
    let mod_name = root
...
pub fn inspect_rpcs3_ordinary_mod(
    source_path: &Path,
) -> Result<Rpcs3OrdinaryModInspection, Rpcs3OrdinaryModError> {
...
pub fn inspect_rpcs3_ordinary_mod_zip(
    source_path: &Path,
) -> Result<Rpcs3OrdinaryModArchiveInspection, Rpcs3OrdinaryModError> {
    let metadata = fs::symlink_metadata(source_path)
        .map_err(|e| error(Rpcs3OrdinaryModErrorKind::SourceNotFound, e.to_string()))?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::SourceNotRegular,
            "RPCS3 mod archive must be a regular file",
        ));
    }
    if metadata.len() > ARCHIVE_MAX_BYTES {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::ResourceLimit,
            "RPCS3 mod archive exceeds the compressed-size limit",
        ));
    }
    ...
    fs::create_dir(&staging_root)
        .map_err(|e| error(Rpcs3OrdinaryModErrorKind::ArchiveRejected, e.to_string()))?;
    ...
    let extracted = crate::dat::archive::zip::extract_zip_members_to(
        source_path,
        &trusted,
        &limits,
    ...
    if extracted.is_empty() {
        ...
        return Err(error(
            Rpcs3OrdinaryModErrorKind::EmptyPackage,
            "RPCS3 mod archive contains no files",
        ));
    }
    ...
    Ok(Rpcs3OrdinaryModArchiveInspection {
...
pub fn rpcs3_ordinary_mod_destination_root(
    profile: &Rpcs3Profile,
    title_id: &str,
) -> Result<PathBuf, Rpcs3OrdinaryModError> {
    if !profile.eligible {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::ProfileUnavailable,
            "RPCS3 profile is not eligible",
        ));
    }
    let title_id = title_id.trim().to_ascii_uppercase();
    if title_id.len() != 9 || !title_id.bytes().all(|byte| byte.is_ascii_alphanumeric()) {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::IdentityUnavailable,
            "a verified PS3 title ID is required",
        ));
    }
    let root = profile.games_path.join(title_id);
    if !root.is_absolute() || root.parent().is_none() {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::ProfileUnsafe,
            "derived RPCS3 game destination is unsafe",
        ));
    }
    Ok(root)
}
...
pub fn build_rpcs3_ordinary_mod_plan(
    ...
    if identity.platform != IdentityPlatform::PlayStation3 {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::IdentityUnavailable,
            "the selected game is not verified as PlayStation 3",
        ));
    }
    let Some(title_id) = identity
        ...
        return Err(error(
            Rpcs3OrdinaryModErrorKind::IdentityUnavailable,
            "a verified PS3 title ID is required; EmuWiz will not guess from names",
        ));
    };
    if inspection.declared_title_id != title_id {
        return Err(error(
            Rpcs3OrdinaryModErrorKind::IdentityConflict,
            "mod PARAM.SFO TITLE_ID conflicts with the selected game's verified title ID",
        ));
    }
...
pub fn build_rpcs3_ordinary_mod_transaction_plan(
    plan: &Rpcs3OrdinaryModPlan,
    profile_id: &str,
) -> Result<SharedTransactionPlan, Rpcs3OrdinaryModError> {
    let mut transaction = build_shared_transaction_plan(
        &plan.report,
        profile_id,
        RPCS3_ORDINARY_MOD_SOURCE_MODE,
        &plan.inspection.source_root,
    )
    .map_err(|e| error(Rpcs3OrdinaryModErrorKind::PreviewFailed, e.detail))?;
    super::shared_transaction::require_rpcs3_ordinary_mod_verification(&mut transaction)
        .map_err(|e| error(Rpcs3OrdinaryModErrorKind::PreviewFailed, e.detail))?;
    Ok(transaction)
}

pub fn apply_rpcs3_ordinary_mod(
    plan: &SharedTransactionPlan,
    options: &SharedApplyOptions,
) -> Rpcs3OrdinaryModApplyResult {
    Rpcs3OrdinaryModApplyResult {
        apply: execute_shared_apply(plan, options),
        rollback: None,
    }
}

pub fn preview_rpcs3_ordinary_mod_rollback(
    journal_path: &Path,
    destination_root: &Path,
    backup_root: &Path,
...
pub fn rollback_rpcs3_ordinary_mod(
