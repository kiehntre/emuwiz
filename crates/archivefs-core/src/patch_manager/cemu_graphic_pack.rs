...
use super::cemu_local::{
    CemuInstallationType, CemuProfile, CemuTitleKind, classify_title_kind, extract_title_identity,
    inspect_extracted_layout,
};
...
fn parse_rules(path: &Path) -> Result<(String, Vec<String>), CemuGraphicPackError> {
    ...
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(error(
            ...
            "rules.txt is not a regular file",
        ));
    }
    if metadata.len() > CEMU_GRAPHIC_PACK_MAX_RULES_BYTES {
        return Err(error(
            CemuGraphicPackErrorKind::ResourceLimit,
            "rules.txt exceeds the inspection limit",
        ));
    }
    let text = String::from_utf8(
        fs::read(path)
            .map_err(|e| error(CemuGraphicPackErrorKind::MalformedRules, e.to_string()))?,
    )
    .map_err(|_| {
        error(
            CemuGraphicPackErrorKind::MalformedRules,
            "rules.txt is not UTF-8",
        )
    })?;
    ...
    for raw in text.lines() {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            in_definition = line[1..line.len() - 1]
                .trim()
            ...
            for id in value.split([',', ' ', '\t']) {
                if !id.is_empty() {
                    if !valid_title_id(id) {
                        return Err(error(
                            CemuGraphicPackErrorKind::MalformedRules,
                            format!("invalid Wii U title ID in rules.txt: {id}"),
                        ));
                    }
    ...
    let name = name.ok_or_else(|| {
        error(
            CemuGraphicPackErrorKind::MalformedRules,
            "rules.txt has no [Definition] name",
        )
    })?;
    if !safe_component(&name) {
        return Err(error(
            CemuGraphicPackErrorKind::UnsafePackName,
            "graphic-pack name is not one safe folder name",
        ));
    }
    if ids.is_empty() {
        return Err(error(
            CemuGraphicPackErrorKind::TargetIdentityMissing,
            "rules.txt declares no base-game titleIds",
        ));
    }
...
fn inspect_root(
    source_path: &Path,
    root: &Path,
) -> Result<CemuGraphicPackInspection, CemuGraphicPackError> {
    let rules = root.join("rules.txt");
    if !rules.exists() {
        return Err(error(
            CemuGraphicPackErrorKind::MissingRules,
            "graphic pack root must contain rules.txt",
        ));
    }
    ...
    fn walk(
        ...
        total: &mut u64,
    ) -> Result<(), CemuGraphicPackError> {
        let mut entries: Vec<_> = fs::read_dir(current)
            .map_err(|e| error(CemuGraphicPackErrorKind::SourceNotFound, e.to_string()))?
        ...
        for entry in entries {
            ...
            if metadata.file_type().is_symlink() || (!metadata.is_file() && !metadata.is_dir()) {
                return Err(error(
                    CemuGraphicPackErrorKind::UnsafeMember,
                    format!("unsafe graphic-pack member: {}", path.display()),
                ));
            }
            if metadata.is_dir() {
                ...
                let relative = path
                    ...
                    .map_err(|_| {
                        error(
                            CemuGraphicPackErrorKind::UnsafeMember,
                            "member escaped graphic-pack root",
                        )
                    })?
                    .to_path_buf();
                if relative
                    ...
                    .any(|c| !matches!(c, Component::Normal(_)))
                {
                    return Err(error(
                        CemuGraphicPackErrorKind::UnsafeMember,
                        "graphic-pack member contains unsafe path components",
                    ));
                }
                if files.len() >= CEMU_GRAPHIC_PACK_MAX_FILES {
                    return Err(error(
                        CemuGraphicPackErrorKind::ResourceLimit,
                        "graphic-pack file-count limit reached",
                    ));
                }
                *total = total.saturating_add(metadata.len());
                if *total > CEMU_GRAPHIC_PACK_MAX_TOTAL_BYTES {
                    return Err(error(
                        CemuGraphicPackErrorKind::ResourceLimit,
                        "graphic-pack expanded-size limit reached",
                    ));
                }
                files.push(relative);
            }
...
fn locate_root(source_path: &Path) -> Result<PathBuf, CemuGraphicPackError> {
    if !source_path.is_absolute() {
        return Err(error(
            CemuGraphicPackErrorKind::SourceNotFound,
            "graphic-pack source path must be absolute",
        ));
    }
    ...
    if metadata.file_type().is_symlink() {
        return Err(error(
            CemuGraphicPackErrorKind::SourceSymlink,
            "graphic-pack source is a symlink",
        ));
    }
    if !metadata.is_dir() {
        return Err(error(
            CemuGraphicPackErrorKind::SourceNotRegular,
            "graphic-pack source must be a folder or ZIP",
        ));
    }
    ...
    {
        let entry =
            entry.map_err(|e| error(CemuGraphicPackErrorKind::SourceNotFound, e.to_string()))?;
        if entry
            .file_type()
            .map_err(|e| error(CemuGraphicPackErrorKind::SourceNotFound, e.to_string()))?
            .is_dir()
            && entry.path().join("rules.txt").is_file()
        {
    ...
    if roots.len() == 1 {
        ...
        Err(error(
            CemuGraphicPackErrorKind::MissingRules,
            "no unambiguous folder containing rules.txt was found",
        ))
    } else {
        Err(error(
            CemuGraphicPackErrorKind::AmbiguousPackRoot,
            "package contains multiple graphic-pack roots",
        ))
    }
}

pub fn inspect_cemu_graphic_pack(
    source_path: &Path,
) -> Result<CemuGraphicPackInspection, CemuGraphicPackError> {
...
pub fn inspect_cemu_graphic_pack_zip(
    source_path: &Path,
) -> Result<CemuGraphicPackArchiveInspection, CemuGraphicPackError> {
    let metadata = fs::symlink_metadata(source_path)
        .map_err(|e| error(CemuGraphicPackErrorKind::SourceNotFound, e.to_string()))?;
    if metadata.file_type().is_symlink() || !metadata.is_file() {
        return Err(error(
            CemuGraphicPackErrorKind::SourceNotRegular,
            "graphic-pack archive must be a regular file",
        ));
    }
    if metadata.len() > ARCHIVE_MAX_BYTES {
        return Err(error(
            CemuGraphicPackErrorKind::ResourceLimit,
            "graphic-pack archive exceeds the compressed-size limit",
        ));
    }
    let staging_root = std::env::temp_dir().join(format!(
        "archivefs-cemu-graphic-pack-{}-{}",
        std::process::id(),
        current_stage_id()
    ));
    fs::create_dir(&staging_root)
        .map_err(|e| error(CemuGraphicPackErrorKind::ArchiveRejected, e.to_string()))?;
    ...
    let members = crate::dat::archive::zip::extract_zip_members_to(
        source_path,
        &trusted,
        &limits,
    ...
    .map_err(|e| {
        ...
        error(
            CemuGraphicPackErrorKind::ArchiveRejected,
            format!("ZIP refused: {e:?}"),
        )
    })?;
    if members.is_empty() {
        let _ = fs::remove_dir_all(&staging_root);
    ...
    Ok(CemuGraphicPackArchiveInspection {
...
pub fn cemu_graphic_packs_root(profile: &CemuProfile) -> Result<PathBuf, CemuGraphicPackError> {
    if !profile.eligible {
        return Err(error(
            CemuGraphicPackErrorKind::ProfileUnavailable,
            profile
                .blocker
                .clone()
                .unwrap_or_else(|| "Cemu profile is not eligible".into()),
        ));
    }
    let root = match profile.installation_type {
        CemuInstallationType::Native => std::env::var_os("HOME")
            .map(PathBuf::from)
            .ok_or_else(|| {
                error(
                    CemuGraphicPackErrorKind::ProfileUnsafe,
                    "HOME is unavailable",
                )
            })?
            .join(".local/share/Cemu/graphicPacks"),
        CemuInstallationType::FlatpakUser => std::env::var_os("HOME")
            .map(PathBuf::from)
            .ok_or_else(|| {
                error(
                    CemuGraphicPackErrorKind::ProfileUnsafe,
            ...
            .join(".var/app/info.cemu.Cemu/data/Cemu/graphicPacks"),
        CemuInstallationType::Portable | CemuInstallationType::Explicit => {
            profile.configuration_path.join("graphicPacks")
        }
    };
    if !root.is_absolute() || root.parent().is_none() {
        return Err(error(
            CemuGraphicPackErrorKind::ProfileUnsafe,
            "derived Cemu graphicPacks path is unsafe",
        ));
    }
    Ok(root)
}
...
pub fn build_cemu_graphic_pack_plan(
    ...
    if !valid_title_id(&selected_title_id) {
        return Err(error(
            CemuGraphicPackErrorKind::TargetIdentityMissing,
            "a verified Cemu title ID is required",
        ));
    }
    if !inspection.title_ids.contains(&selected_title_id) {
        return Err(error(
            CemuGraphicPackErrorKind::TargetNotDeclared,
            "the pack does not declare the selected game's title ID",
        ));
    }
    let source_items = inspection
        .files
        .iter()
        .map(|relative| PreviewSourceItem {
    ...
    let report = build_shared_preview(&SharedPreviewRequest {
        ...
        identity: PreviewIdentity {
            kind: PreviewIdentityKind::CemuTitleId,
            state: PreviewIdentityState::Verified,
            value: Some(selected_title_id.clone()),
    ...
    Ok(CemuGraphicPackPlan {
        inspection: inspection.clone(),
        target_title_id: selected_title_id,
        destination_root: destination_root.to_path_buf(),
        report,
    })
}

pub fn build_cemu_graphic_pack_transaction_plan(
    plan: &CemuGraphicPackPlan,
    profile_id: &str,
) -> Result<SharedTransactionPlan, CemuGraphicPackError> {
    let mut transaction = build_shared_transaction_plan(
        &plan.report,
        profile_id,
        CEMU_GRAPHIC_PACK_SOURCE_MODE,
...
pub fn apply_cemu_graphic_pack(
    plan: &SharedTransactionPlan,
    options: &SharedApplyOptions,
) -> CemuGraphicPackApplyResult {
...
pub fn preview_cemu_graphic_pack_rollback(
    journal_path: &Path,
    destination_root: &Path,
    backup_root: &Path,
...
pub fn rollback_cemu_graphic_pack(
