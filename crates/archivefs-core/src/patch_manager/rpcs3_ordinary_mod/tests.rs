...
fn valid_wrapper_preserves_nested_layout_and_requires_package_identity() {
    ...
    assert_eq!(inspection.files, vec![PathBuf::from("PS3_GAME/USRDIR/mod/nested/file.bin")]);
    let selected = dir.path().join("game.iso");
    let identity = report(TITLE, &selected);
    let plan = build_rpcs3_ordinary_mod_plan(&inspection, &identity, &dir.path().join("game-root")).unwrap();
    assert_eq!(plan.report.entries[0].destination_relative_path.as_deref().unwrap(), Path::new("USRDIR/mod/nested/file.bin"));
}
...
fn missing_malformed_and_empty_packages_fail_closed() {
    ...
    assert_eq!(inspect_rpcs3_ordinary_mod(&empty).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::MissingIdentity);
    ...
    assert_eq!(inspect_rpcs3_ordinary_mod(&malformed).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::MalformedIdentity);
    ...
    assert_eq!(inspect_rpcs3_ordinary_mod(&identity_only).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::EmptyPackage);
}
...
fn conflicting_identity_and_selected_identity_are_refused() {
    ...
    assert_eq!(inspect_rpcs3_ordinary_mod(&root).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::ConflictingIdentity);
    ...
    assert_eq!(build_rpcs3_ordinary_mod_plan(&inspection, &identity, &dir.path().join("game-root")).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::IdentityConflict);
}
...
fn zip_traversal_and_symlink_members_are_rejected() {
    ...
    zip.finish().unwrap();
    assert_eq!(inspect_rpcs3_ordinary_mod_zip(&archive).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::ArchiveRejected);

    #[cfg(unix)] {
        std::os::unix::fs::symlink(dir.path().join("missing"), dir.path().join("link" )).unwrap();
        assert_eq!(inspect_rpcs3_ordinary_mod(&dir.path().join("link")).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::SourceSymlink);
        ...
        .unwrap();
        assert_eq!(inspect_rpcs3_ordinary_mod(&hardlink_root).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::UnsafeMember);
        ...
        assert_eq!(inspect_rpcs3_ordinary_mod(&fifo_root).unwrap_err().kind, Rpcs3OrdinaryModErrorKind::UnsafeMember);
    }
}
...
fn identical_files_skip_and_apply_rollback_restores_replaced_files() {
    ...
    let applied = apply_rpcs3_ordinary_mod(&transaction, &SharedApplyOptions {
        dry_run: false,
        ...
        operation_id: "rpcs3-mod-apply".into(), timestamp_unix_seconds: 1,
        current_context: transaction.context.clone(), history_root: history.clone(), backup_root: backups.clone(),
    });
    ...
    let rollback = rollback_rpcs3_ordinary_mod(&preview, &SharedRollbackOptions {
        confirmation: SharedRollbackConfirmation { preview_id: preview.preview_id.clone(), approved: true },
        rollback_operation_id: "rpcs3-mod-rollback".into(), timestamp_unix_seconds: 2,
        history_root: history, backup_root: backups,
    });
...
fn stale_source_is_rejected_before_write() {
    ...
    let plan = build_rpcs3_ordinary_mod_plan(&inspection, &identity, &destination).unwrap();
    let transaction = build_rpcs3_ordinary_mod_transaction_plan(&plan, "rpcs3:test").unwrap();
    fs::write(root.join("PS3_GAME/USRDIR/mod/nested/file.bin"), b"changed after review").unwrap();
    let result = apply_rpcs3_ordinary_mod(&transaction, &SharedApplyOptions {
        dry_run: false,
        confirmation: Some(SharedApplyConfirmation { plan_id: transaction.plan_id.clone(), general_approved: true, replacement_approved: true }),
        operation_id: "rpcs3-stale-source".into(), timestamp_unix_seconds: 1,
        current_context: transaction.context.clone(), history_root: dir.path().join("history"), backup_root: dir.path().join("backups"),
    });
