...
//! platform, a direct regular `.iso`, `.cue`, or `.chd` file, a verified
//! Dreamcast product code, and an exact eligible
//! [`FlycastNativeLaunchBinding`]. Mounted/archive content, GDI/CDI, and
//! multi-track GD-ROM images are all refused here - never silently widened.
//!
//! `.iso`, `.cue`, and `.chd` are exactly the formats
...
//! that module) - this slice never accepts a format the identity layer
//! could not already prove. GDI/CDI/multi-track GD-ROM have no such
//! authoritative identity path today and remain deferred, matching the
...
/// identity check already verifies authoritatively. GDI/CDI and other
/// multi-track GD-ROM representations are deliberately absent.
const FLYCAST_SUPPORTED_EXTENSIONS: &[&str] = &["iso", "cue", "chd"];
...
pub fn build_flycast_command_plan(
    ...
    if let Some(path) = &content_path {
        if crate::archive_kind(path).is_some_and(|kind| kind.is_mount_input()) {
            ...
            blockers.push(blocker(
                LaunchBlockerKind::FlycastContentFormatUnsupported,
                "only a direct .iso, .cue, or .chd file is supported by this native Flycast \
                 launch slice; GDI/CDI and multi-track GD-ROM are not supported",
            ));
        }
    }
