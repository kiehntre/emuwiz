...
impl ArchiveFsApp {
    pub(crate) fn start_platform_artwork_task(
        &mut self,
        ...
        if matches!(action, PlatformArtworkManagerAction::OpenFolder) {
            ...
            {
                self.artwork_media.platform_artwork_manager.message =
    ...
    pub(crate) fn poll_platform_artwork_task(&mut self, context: &egui::Context) {
        ...
        match result {
            PlatformArtworkTaskResult::Status(result) => match result {
                ...
                Err(error) => {
                    self.artwork_media.platform_artwork_manager.message = Some((false, error))
                }
            },
